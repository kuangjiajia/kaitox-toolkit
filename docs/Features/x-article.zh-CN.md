...
打开 <https://x.com/compose/articles>，在 Kaitox 面板里找到你的草稿，点**上传草稿**。如果你在 Chrome 插件设置里开启了「跳转到页面立即自动上传」，`kaitox x push` 打印的自动上传 URL 或 Obsidian 打开的跳转链接会按草稿 id 立刻开始这一步。插件会用你自己已登录的会话上传图片、创建 Article 草稿，然后跳进编辑器。图片和排版一次到位——满意后在 X 里自行发布。
