...
3. 点上传。如果插件设置里开启了「跳转到页面立即自动上传」，也可以打开 `kaitox x push` 打印的自动上传 URL；它带有草稿 id，会立刻开始这一步。
4. 插件用页面自己的登录会话上传图片并创建 Article 草稿——原始 Markdown 到这一步才被转换成 X 的内容格式，因为图片的 `media_id` 只有在登录页面里上传之后才存在。
5. `kaitox x status <id>` 会反映结果，包括文章的 `rest_id`。
